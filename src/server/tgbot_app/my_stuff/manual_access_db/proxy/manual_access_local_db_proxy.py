...
class ManualAccessLocalDBProxy(ManualAccessDB):
    ...
    def total_cleanup(self, msg: TxtMsg):
        ...
        if self._check_access(msg):
            response.send_message(msg.chat_id, "Conducting cleanup...")
            self._db_manager.total_cleanup(msg)
            response.send_message(msg.chat_id, "Cleanup is successfully done")
        else:
            response.send_message(msg.chat_id, "You don't have admin privileges")
